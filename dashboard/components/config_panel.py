        ...
        with st.expander("Parameters", expanded=True):
            for spec in workflow.accepted_arguments:
                ...
                if spec.kind == "bool":
                    ...
                    arguments[spec.name] = st.text_input(spec.flag, value=default, help=help_text, key=f"arg-{workflow.key}-{spec.name}")
    ...
    if workflow.input_assignments:
        with st.expander("Input assignments", expanded=True):
            ...
            for spec in workflow.input_assignments:
                selected = st.selectbox(spec.label, labels, help=spec.description or None, key=f"input-{workflow.key}-{spec.role}")
                custom = st.text_input(f"Existing path for {spec.label}", value="", key=f"input-path-{workflow.key}-{spec.role}")
                if custom.strip():
                    assignments[spec.role] = custom.strip()
                elif selected != "Do not pass":
                    assignments[spec.role] = selected

    config_paths = [Path(path) for path in assignments.values() if Path(path).suffix.lower() in {".json", ".yaml", ".yml", ".toml", ".txt"}]
    if config_paths:
