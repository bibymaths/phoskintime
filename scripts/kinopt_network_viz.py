...
Inputs (generated by kinopt_network_readout.py):
  - results_scripts/figures_kinopt/kinopt_kinase_load.csv
  - results_scripts/figures_kinopt/kinopt_target_dominant_kinases.csv
  - results_scripts/figures_kinopt/kinopt_knockout_effects.csv

Outputs:
  - results_scripts/figures_kinopt/*.pdf and *.svg
...
2) Kinase breadth vs control load:
   - Label kinases with control load > 1.0
...
def plot_target_fragility(td: pd.DataFrame):
    ...
    # Label outliers: points outside the 95% band (largest deviations from identity)
    out_of_band = d[d["perp_dist"] > thr].copy()
    label_df = out_of_band.sort_values("perp_dist", ascending=False).head(OUTLIER_MAX_LABELS)
