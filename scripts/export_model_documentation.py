...
from export_model_to_sbml import FAMILIES, SOURCE_HINTS, EQUATIONS, discover_model_outputs, ModelResult  # noqa: E402
...
def render_latex(docs: list[ModelDocumentation], results_dir: Path) -> str:
    ...
    for d in docs:
        ...
        out += [r"\end{longtable}", r"\subsection*{Constraints and assumptions}", esc(CONSTRAINTS.get(fam, "No family-specific constraints reconstructed.")), " Unitless/fold-change observables are dimensionless; no absolute concentration units are implied."]
...
def main(argv=None) -> int:
    ...
    results=discover_model_outputs(a.results_dir, a.model_family, a.run_id); docs=[]; found={r.model_family for r in results}
    for fam in FAMILIES:
