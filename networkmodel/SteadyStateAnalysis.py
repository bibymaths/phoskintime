...
from networkmodel.simulate import simulate_diffrax, simulate_pinn_diffrax
from config.config import setup_logger

logger = setup_logger(log_dir=RESULTS_DIR)

def _pinn_mode(pinn_config) -> str:
    if pinn_config is None:
        return "off"
    return str(getattr(pinn_config, "mode", "off")).lower()


def _use_pinn_simulator(theta=None, slices=None, pinn_config=None, pinn_spec=None) -> bool:
    mode = _pinn_mode(pinn_config)
    return (
        mode in {"hybrid", "neuralode"}
...
def _is_pure_neuralode(pinn_config) -> bool:
    return _pinn_mode(pinn_config) == "neuralode"

def simulate_until_steady(
    sys,
    t_max=1440.0,
    n_points=1000,
    *,
    ...
    PINN behavior:
      - off       -> mechanistic simulate_diffrax
      - hybrid    -> simulate_pinn_diffrax with mechanistic + neural RHS
      - neuralode -> simulate_pinn_diffrax with neural RHS only
    """
    ...
    if _use_pinn_simulator(
        ...
        logger.info(
            "[SteadyState] Simulating PINN mode=%s for %g minutes...",
            mode,
            float(t_max),
        )
        ...
        logger.info("[SteadyState] Simulating mechanistic system for %g minutes...", float(t_max))
    ...
    # Use finite difference for all modes. This avoids calling sys.rhs,
    # which is mechanistic-only and wrong for PINN/neuralode.
    dt = t_eval[-1] - t_eval[-2]
    dist = np.linalg.norm(Y[-1] - Y[-2])
    rate = dist / max(dt, 1e-12)

    logger.info("[SteadyState] Final finite-difference rate of change: %.2e", rate)
    logger.info("[SteadyState] Simulate until steady-state [Done].")

    return t_eval, Y


def plot_steady_state_all(t, Y, sys, idx, output_dir, *, pinn_config=None):
    ...
    mode = _pinn_mode(pinn_config)
    pure_neuralode = _is_pure_neuralode(pinn_config)
    ...
    if pure_neuralode:
        if len(t) >= 2:
            dt_last = float(t[-1] - t[-2])
            dy_last = (Y[-1] - Y[-2]) / max(dt_last, 1e-12)
        else:
            dy_last = np.zeros_like(y_last)
    ...
    if pure_neuralode:
        logger.info(
            "[SteadyState] Skipping kinase-drive and S_all steady-state summaries "
            "for pure NeuralODE mode; no mechanistic kinase RHS is active."
        )
    else:
