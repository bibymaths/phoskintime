...
def load_data(args):
    ...
    # 3) MS (protein + phospho-sites in SAME file)
    ...
    if len(xcols) >= 2:
        ...
        tidy["psite"] = tidy[scol].fillna("").astype(str).str.strip().replace({"nan": "", "NaN": ""})
        ...
        tidy["psite"] = tidy["psite"].fillna("").astype(str).str.strip().replace({"nan": "", "NaN": ""})
    ...
    if df_pho.empty:
        df_pho = pd.DataFrame(columns=["protein", "psite", "time", "fc"])
    ...
    # CHANGED RETURN: include df_pho
    return df_kin_clean, df_tf_clean, df_prot, df_pho, df_rna
